...
const loginUser = asyncHandler(async (req, res) => {
  ...
  // httpOnly and secure is kept true so that no one can modify cookies and only server has the authority to do so
  const options = {
    httpOnly: true,
    secure: true,
  };

  // Response that is sent to the user after the login is finished
  return res
    .status(200)
...
const getCurrentUser = asyncHandler(async (req, res) => {
  return res
    .status(200)
    .json(new ApiResponse(200, req.user, "Current User Fetched Successufully"));
});
